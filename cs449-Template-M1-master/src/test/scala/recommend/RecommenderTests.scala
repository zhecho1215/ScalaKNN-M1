...
class RecommenderTests extends AnyFunSuite with BeforeAndAfterAll {
  ...
  var solver: RecommenderSolver = null
  ...
  override def beforeAll {
    ...
    solver = new RecommenderSolver(data ++ personal, k = 300)
    predictor = solver.personalizedPredictor(_, _)
  }
  ...
  test("Top 3 recommendations for user 944") {
    val recommendations = solver.getRecommendations(user=944, top=3)
