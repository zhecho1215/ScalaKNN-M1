...
class DistributedBaselineTests extends AnyFunSuite with BeforeAndAfterAll {
   ...
   test("Compute global average")                           { assert(within(solver.globalAvg(train2), 3.5264625, 0.0001)) }
   ...
   test("Compute baseline prediction for user 1 on item 1") { assert(within(solver.getSingleBaselinePrediction(train2)(1,1), 4.046819980619529, 0.0001)) }
